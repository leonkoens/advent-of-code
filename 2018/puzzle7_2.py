...
class Graph:
    ...
    def resolve(self):
        ...
        while zero_edges or sum(elves) != 0:

            if len(elves) == elves_len:
                
                while min(elves.values()) != 0:
                    for key in elves:
                        elves[key] -= 1
                
            for node in zero_edges:
                
            len(eleves) != elves_len and 
            
            available_elf = elves.index(0)

            zero_edges = sorted(zero_edges)
            n = zero_edges.pop(0)

            elves[available_elf] += (string.ascii_uppercase.index(n)+1)
...
content = ''
with open('input7_test.txt') as handle:
