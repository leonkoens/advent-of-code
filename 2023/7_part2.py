...
class AdventOfCode:
    ...
    def run(self):
        ...
        def type_score(hand):
            hand_count = Counter(list(hand.split(' ')[0]))

            if 'J' in hand_count:
                j_count = hand_count.pop('J')

                try:
                    max_key = max(hand_count, key=hand_count.get)
                except ValueError:
                    return 7

                hand_count[max_key] += j_count

            hand_count = hand_count.values()
        ...
        def compare_hands(a, b):
            ...
            if a_type_score == b_type_score:
                ...
                a_type_score = type_score(a.replace('J', 'X'))
                b_type_score = type_score(a.replace('J', 'X'))
